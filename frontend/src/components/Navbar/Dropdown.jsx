...
export default function Dropdown({ handleLogout }) {
  ...
  return (
    <div className="absolute top-4 right-[18%] z-40 w-56 text-right">
      <Menu as="div" className="relative inline-block text-left">
        <div>
          ...
          <div className="w-8 h-8 border border-white border-opacity-30 rounded-[50%] flex justify-center items-center bg-white bg-opacity-10">
              <p className="font-bold text-[17px]" >{user.username.charAt(0).toUpperCase()}</p>
            </div>
            <ChevronDownIcon
              className="ml-2 -mr-1 h-5 w-5 text-pink-200 hover:text-pink-100"
              aria-hidden="true"
            />
          </Menu.Button>
        </div>
        ...
        >
          <Menu.Items className="absolute right-0 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-black bg-opacity-20 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
            <div className="px-1 py-1 ">
              <Menu.Item>
                {({ active }) => (
                  <button
                    className={`${
                      active ? "bg-[#fe24c0] text-white" : "text-white"
